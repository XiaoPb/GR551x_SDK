...
# Table of contents
########################################################
# 1. Sdk-aware extensions
# 1.1. sdk_*
# 1.2. sdk_library_*
# 1.2.1 sdk_interface_library_*
# 1.3. generate_inc_*
# 1.4. board_*
# 1.5. Misc.
...
# "sdk". sdk is a catch-all CMake library for source files that
# can be built purely with the include paths, defines, and other
# compiler flags that all sdk source files use.
...
#
#  sdk_library_cc_option()
#           |
#           v
#  sdk_library_compile_options()  -->  target_compile_options()
#
#
#  sdk_cc_option()           --->  target_cc_option()
#                                          |
#                                          v
#  sdk_cc_option_fallback()  --->  target_cc_option_fallback()
#                                          |
#                                          v
#  sdk_compile_options()     --->  target_compile_options()
#


# https://cmake.org/cmake/help/latest/command/target_sources.html
function(sdk_sources)
  foreach(arg ${ARGV})
    if(IS_DIRECTORY ${arg})
      message(FATAL_ERROR "sdk_sources() was called on a directory")
    endif()
    target_sources(sdk PRIVATE ${arg})
    # target_sources(${APPLICATION_NAME_ELF} PRIVATE ${arg})
  endforeach()
endfunction()
...
function(application_sources)
  foreach(arg ${ARGV})
    if(IS_DIRECTORY ${arg})
      message(FATAL_ERROR "application_sources() was called on a directory")
    endif()
    # target_sources(sdk PRIVATE ${arg})
    target_sources(${APPLICATION_NAME_ELF} PUBLIC ${arg})
  endforeach()
endfunction()

# https://cmake.org/cmake/help/latest/command/target_include_directories.html
function(sdk_include_directories)
  target_include_directories(sdk_interface INTERFACE ${ARGV})
endfunction()

# https://cmake.org/cmake/help/latest/command/target_include_directories.html
function(sdk_system_include_directories)
  target_include_directories(sdk_interface SYSTEM INTERFACE ${ARGV})
endfunction()

# https://cmake.org/cmake/help/latest/command/target_compile_definitions.html
function(sdk_compile_definitions)
  target_compile_definitions(sdk_interface INTERFACE ${ARGV})
endfunction()

# https://cmake.org/cmake/help/latest/command/target_compile_options.html
function(sdk_compile_options)
  target_compile_options(sdk_interface INTERFACE ${ARGV})
endfunction()

# https://cmake.org/cmake/help/latest/command/target_link_libraries.html
function(sdk_link_libraries)
  target_link_libraries(sdk_interface INTERFACE ${ARGV})
endfunction()

function(sdk_libc_link_libraries)
  set_property(TARGET sdk_interface APPEND PROPERTY LIBC_LINK_LIBRARIES ${ARGV})
endfunction()

# See this file section 3.1. target_cc_option
function(sdk_cc_option)
  foreach(arg ${ARGV})
    target_cc_option(sdk_interface INTERFACE ${arg})
  endforeach()
endfunction()

function(sdk_cc_option_fallback option1 option2)
    target_cc_option_fallback(sdk_interface INTERFACE ${option1} ${option2})
endfunction()

function(sdk_ld_options)
    target_ld_options(sdk_interface INTERFACE ${ARGV})
endfunction()

# Getter functions for extracting build information from
# sdk_interface. Returning lists, and strings is supported, as is
# requesting specific categories of build information (defines,
# includes, options).
#
# The naming convention follows:
# sdk_get_${build_information}_for_lang${format}(lang x [STRIP_PREFIX])
# Where
...
#
# e.g.
# sdk_get_include_directories_for_lang(ASM x)
# writes "-Isome_dir;-Isome/other/dir" to x

function(sdk_get_include_directories_for_lang_as_string lang i)
  sdk_get_include_directories_for_lang(${lang} list_of_flags DELIMITER " " ${ARGN})
...
function(sdk_get_system_include_directories_for_lang_as_string lang i)
  sdk_get_system_include_directories_for_lang(${lang} list_of_flags DELIMITER " " ${ARGN})
...
function(sdk_get_compile_definitions_for_lang_as_string lang i)
  sdk_get_compile_definitions_for_lang(${lang} list_of_flags DELIMITER " " ${ARGN})
...
function(sdk_get_compile_options_for_lang_as_string lang i)
  sdk_get_compile_options_for_lang(${lang} list_of_flags DELIMITER " ")
...
function(sdk_get_include_directories_for_lang lang i)
  sdk_get_parse_args(args ${ARGN})
  get_property(flags TARGET sdk_interface PROPERTY INTERFACE_INCLUDE_DIRECTORIES)
...
function(sdk_get_system_include_directories_for_lang lang i)
  sdk_get_parse_args(args ${ARGN})
  get_property(flags TARGET sdk_interface PROPERTY INTERFACE_SYSTEM_INCLUDE_DIRECTORIES)
...
function(sdk_get_compile_definitions_for_lang lang i)
  sdk_get_parse_args(args ${ARGN})
  get_property(flags TARGET sdk_interface PROPERTY INTERFACE_COMPILE_DEFINITIONS)
...
function(sdk_get_compile_options_for_lang lang i)
  sdk_get_parse_args(args ${ARGN})
  get_property(flags TARGET sdk_interface PROPERTY INTERFACE_COMPILE_OPTIONS)
...
#
# Usage:
#   sdk_get_parse_args(foo ${ARGN})
#   print(foo_STRIP_PREFIX) # foo_STRIP_PREFIX might be set to 1
function(sdk_get_parse_args return_dict)
  foreach(x ${ARGN})
...
# 1.2 sdk_library_*
#
# Sdk libraries use CMake's library concept and a set of
# assumptions about how sdk code is organized to cut down on
# boilerplate code.
#
# A Sdk library can be constructed by the function sdk_library
# or sdk_library_named. The constructors create a CMake library
# with a name accessible through the variable SDK_CURRENT_LIBRARY.
#
# The variable SDK_CURRENT_LIBRARY should seldom be needed since
# the sdk libraries have methods that modify the libraries. These
# methods have the signature: sdk_library_<target-function>
#
...
#  PRIVATE is assumed
#  The target is assumed to be SDK_CURRENT_LIBRARY
#
# When a flag that is given through the sdk_* API conflicts with
# the sdk_library_* API then precedence will be given to the
# sdk_library_* API. In other words, local configuration overrides
# global configuration.

# Constructor with a directory-inferred name
macro(sdk_library)
  sdk_library_get_current_dir_lib_name(${SDK_BASE} lib_name)
  sdk_library_named(${lib_name})
endmacro()
...
macro(sdk_library_get_current_dir_lib_name base lib_name)
  # Remove the prefix (/home/sebo/sdk/driver/serial/CMakeLists.txt => driver/serial/CMakeLists.txt)
  ...
  # Replace : with __ (C:/sdkproject => C____sdkproject)
  string(REGEX REPLACE ":" "__" name ${name})
...
macro(sdk_library_named name)
  # This is a macro because we need add_library() to be executed
  # within the scope of the caller.
  set(SDK_CURRENT_LIBRARY ${name})
  add_library(${name} STATIC "")

  sdk_append_cmake_library(${name})

  target_link_libraries(${name} PUBLIC sdk_interface)
endmacro()

# Provides amend functionality to a Sdk library for out-of-tree usage.
#
# When called from a Sdk module, the corresponding sdk library defined
# within Sdk will be looked up.
#
# Note, in order to ensure correct library when amending, the folder structure in the
# Sdk module must resemble the structure used in Sdk, as example:
#
# Example: to amend the sdk library created in
# SDK_BASE/drivers/entropy/CMakeLists.txt
# add the following file:
# SDK_MODULE/drivers/entropy/CMakeLists.txt
# with content:
# sdk_library_amend()
# sdk_library_sources(...)
#
# It is also possible to use generator expression when amending to Sdk
# libraries.
#
# For example, in case it is required to expose the Sdk library's folder as
# include path then the following is possible:
# sdk_library_amend()
# sdk_library_include_directories($<TARGET_PROPERTY:SOURCE_DIR>)
#
# See the CMake documentation for more target properties or generator
# expressions.
#
macro(sdk_library_amend)
  # This is a macro because we need to ensure the SDK_CURRENT_LIBRARY and
  # following sdk_library_* calls are executed within the scope of the
  # caller.
  if(NOT SDK_CURRENT_MODULE_DIR)
    message(FATAL_ERROR "Function only available for Sdk modules.")
  endif()

  sdk_library_get_current_dir_lib_name(${SDK_CURRENT_MODULE_DIR} lib_name)

  set(SDK_CURRENT_LIBRARY ${lib_name})
endmacro()

function(sdk_link_interface interface)
  target_link_libraries(${interface} INTERFACE sdk_interface)
endfunction()

#
# sdk_library versions of normal CMake target_<func> functions
# Note, paths passed to this function must be relative in order
# to support the library relocation feature of sdk_code_relocate
#
function(sdk_library_sources source)
  target_sources(${SDK_CURRENT_LIBRARY} PRIVATE ${source} ${ARGN})
endfunction()

function(sdk_library_include_directories)
  target_include_directories(${SDK_CURRENT_LIBRARY} PRIVATE ${ARGN})
endfunction()

function(sdk_library_link_libraries item)
  target_link_libraries(${SDK_CURRENT_LIBRARY} PUBLIC ${item} ${ARGN})
endfunction()

function(sdk_library_compile_definitions item)
  target_compile_definitions(${SDK_CURRENT_LIBRARY} PRIVATE ${item} ${ARGN})
endfunction()

function(sdk_library_compile_options item)
  ...
  # on the command line. So to ensure that sdk_library_* flags are
  ...
  # the command line than the the flags from sdk_interface because
  # sdk_interface will be the first interface library that flags
  # are taken from.
  ...
  target_link_libraries(${SDK_CURRENT_LIBRARY} PRIVATE ${lib_name})
endfunction()

function(sdk_library_cc_option)
  foreach(option ${ARGV})
    string(MAKE_C_IDENTIFIER check${option} check)
    sdk_check_compiler_flag(C ${option} ${check})

    if(${${check}})
      sdk_library_compile_options(${option})
    endif()
  endforeach()
endfunction()

function(sdk_library_add_dependencies)
  add_dependencies(${SDK_CURRENT_LIBRARY} ${ARGN})
endfunction()

# Add the existing CMake library 'library' to the global list of
# Sdk CMake libraries. This is done automatically by the
# constructor but must be called explicitly on CMake libraries that do
# not use a sdk library constructor.
function(sdk_append_cmake_library library)
  if(TARGET sdk_prebuilt)
    message(WARNING
      "sdk_library() or sdk_library_named() called in Sdk CMake "
      "application mode. `${library}` will not be treated as a Sdk library."
      "To create a Sdk library in Sdk CMake kernel mode consider "
      "creating a Sdk module. See more here: "
      "https://docs.sdkproject.org/latest/guides/modules.html"
    )
  endif()
  set_property(GLOBAL APPEND PROPERTY SDK_LIBS ${library})
endfunction()

# Add the imported library 'library_name', located at 'library_path' to the
# global list of Sdk CMake libraries.
function(sdk_library_import library_name library_path)
  add_library(${library_name} STATIC IMPORTED GLOBAL)
  set_target_properties(${library_name}
    PROPERTIES IMPORTED_LOCATION
    ${library_path}
    )
  sdk_append_cmake_library(${library_name})
endfunction()

# Place the current sdk library in the application memory partition.
#
...
function(sdk_library_app_memory partition)
  set_property(TARGET sdk_property_target
               APPEND PROPERTY COMPILE_OPTIONS
               "-l" $<TARGET_FILE_NAME:${SDK_CURRENT_LIBRARY}> "${partition}")
endfunction()

# Configure a Sdk library specific property.
#
# Usage:
#   sdk_library_property(<property> <value>)
#
# Current Sdk library specific properties that are supported:
# ALLOW_EMPTY <TRUE:FALSE>: Allow a Sdk library to be empty.
#                           An empty Sdk library will generate a CMake
#                           configure time warning unless `ALLOW_EMPTY` is TRUE.
function(sdk_library_property)
  set(single_args "ALLOW_EMPTY")
  cmake_parse_arguments(LIB_PROP "" "${single_args}" "" ${ARGN})

  if(LIB_PROP_UNPARSED_ARGUMENTS)
      message(FATAL_ERROR "sdk_library_property(${ARGV0} ...) given unknown arguments: ${FILE_UNPARSED_ARGUMENTS}")
  endif()

  foreach(arg ${single_args})
    if(DEFINED LIB_PROP_${arg})
      set_property(TARGET ${SDK_CURRENT_LIBRARY} PROPERTY ${arg} ${LIB_PROP_${arg}})
    endif()
  endforeach()
endfunction()
...
# 1.2.1 sdk_interface_library_*
#
# A Sdk interface library is a thin wrapper over a CMake INTERFACE
...
#
# A Sdk interface library should be used when there exists some
# build information (include directories, defines, compiler flags,
# etc.) that should be applied to a set of Sdk libraries and 'app'
# might be one of these libraries.
#
# Sdk libraries must explicitly call
# sdk_library_link_libraries(<interface_library>) to use this build
...
#
# This API has a constructor like the sdk_library API has, but it
# does not have wrappers over the other cmake target functions.
macro(sdk_interface_library_named name)
  add_library(${name} INTERFACE)
  set_property(GLOBAL APPEND PROPERTY SDK_INTERFACE_LIBS ${name})
endmacro()
...
function(generate_inc_file
  ...
  add_custom_command(
    ...
    COMMAND
    ${PYTHON_EXECUTABLE}
    ${SDK_BASE}/scripts/build/file2hex.py
...
# 1.4. board_*
#
# This section is for extensions related to Sdk board handling.
#
# Sdk board extensions current contains:
# - Board runners
# - Board revision

# Sdk board runners:
#   Sdk board runner extension functions control Sdk's board runners
#   from the build system. The Sdk build system has targets for
#   flashing and debugging supported boards. These are wrappers around a
#   "runner" Python subpackage that is part of Sdk's "west" tool.
#
...
# When `EXACT` is not specified, this function will set the Sdk build system
...
#                   with an older Sdk. As example, if current board supports
...
# sdk_check_compiler_flag is a part of Sdk's toolchain
...
function(sdk_check_compiler_flag lang option check)
  # Check if the option is covered by any hardcoded check before doing
  # an automated test.
  sdk_check_compiler_flag_hardcoded(${lang} "${option}" _${check} exists)
  if(exists)
  ...
  set_ifndef(
    SDK_TOOLCHAIN_CAPABILITY_CACHE_DIR
  ...
  # Check the cache
  set(key_path ${SDK_TOOLCHAIN_CAPABILITY_CACHE_DIR}/${key})
  if(EXISTS ${key_path})
    file(READ
  ...
  if(NOT (EXISTS ${key_path}))
    ...
    file(
      APPEND
      ${SDK_TOOLCHAIN_CAPABILITY_CACHE_DIR}/log.txt
      "${inner_check} ${key} ${key_string}\n"
      )
  endif()
endfunction()

function(sdk_check_compiler_flag_hardcoded lang option check exists)
...
endfunction(sdk_check_compiler_flag_hardcoded)

# sdk_linker_sources(<location> [SORT_KEY <sort_key>] <files>)
#
...
#    relative to the calling file, like sdk_sources().
# <location> is one of
...
function(sdk_linker_sources location)
  ...
  foreach(file IN ITEMS ${L_UNPARSED_ARGUMENTS})
    ...
    if(IS_DIRECTORY ${path})
      message(FATAL_ERROR "sdk_linker_sources() was called on a directory")
    endif()

    # Find the relative path to the linker file from the include folder.
    file(RELATIVE_PATH relpath ${SDK_BASE}/include ${path})
  ...
  endforeach()
endfunction(sdk_linker_sources)

# Helper macro for conditionally calling sdk_code_relocate() when a
# specific Kconfig symbol is enabled. See sdk_code_relocate() description
# for supported arguments.
macro(sdk_code_relocate_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_code_relocate(${ARGN})
  endif()
endmacro()
...
# sdk_code_relocate(FILES file1.c file2.c LOCATION SRAM)
...
# sdk_code_relocate(LIBRARY my_lib SRAM)
#
...
function(sdk_code_relocate)
  ...
  if(CODE_REL_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "sdk_code_relocate(${ARGV0} ...) "
      "given unknown arguments: ${CODE_REL_UNPARSED_ARGUMENTS}")
  endif()
  if((NOT CODE_REL_FILES) AND (NOT CODE_REL_LIBRARY))
    message(FATAL_ERROR
      "sdk_code_relocate() requires either FILES or LIBRARY be provided")
  endif()
  if(CODE_REL_FILES AND CODE_REL_LIBRARY)
    message(FATAL_ERROR "sdk_code_relocate() only accepts "
      "one argument between FILES and LIBRARY")
  endif()
  if(NOT CODE_REL_LOCATION)
    message(FATAL_ERROR "sdk_code_relocate() requires a LOCATION argument")
  endif()
  if(CODE_REL_LIBRARY)
...
# Examples
# calling
#   sdk_build_string(build_string BOARD alpha BUILD debug)
# will return the string `alpha_debug` in `build_string` parameter.
#
# calling
#   sdk_build_string(build_string BOARD alpha BOARD_REVISION 1.0.0 BUILD debug)
# will return the string `alpha_1_0_0_debug` in `build_string` parameter.
#
function(sdk_build_string outvar)
  ...
  if(BUILD_STR_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR
      "sdk_build_string(${ARGV0} <val> ...) given unknown arguments:"
  ...
  if(DEFINED BUILD_STR_BOARD_REVISION AND NOT BUILD_STR_BOARD)
    message(FATAL_ERROR
      "sdk_build_string(${ARGV0} <list> BOARD_REVISION ${BUILD_STR_BOARD_REVISION} ...)"
...
function(sdk_syscall_header_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_syscall_header(${ARGN})
  endif()
endfunction()
...
# to Sdk. Primarily they work around limitations in the CMake
...
function(sdk_library_sources_ifdef feature_toggle source)
  if(${${feature_toggle}})
    sdk_library_sources(${source} ${ARGN})
  endif()
endfunction()

function(sdk_sources_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_sources(${ARGN})
  endif()
endfunction()

function(application_sources_ifdef feature_toggle)
  if(${${feature_toggle}})
    application_sources(${ARGN})
  endif()
endfunction()

function(sdk_cc_option_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_cc_option(${ARGN})
  endif()
endfunction()

function(sdk_ld_option_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_ld_options(${ARGN})
  endif()
endfunction()

function(sdk_link_libraries_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_link_libraries(${ARGN})
  endif()
endfunction()

function(sdk_compile_options_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_compile_options(${ARGN})
  endif()
endfunction()

function(sdk_compile_definitions_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_compile_definitions(${ARGN})
  endif()
endfunction()

function(sdk_include_directories_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_include_directories(${ARGN})
  endif()
endfunction()

function(sdk_library_compile_definitions_ifdef feature_toggle item)
  if(${${feature_toggle}})
    sdk_library_compile_definitions(${item} ${ARGN})
  endif()
endfunction()

function(sdk_library_include_directories_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_library_include_directories(${ARGN})
  endif()
endfunction()

function(sdk_library_compile_options_ifdef feature_toggle item)
  if(${${feature_toggle}})
    sdk_library_compile_options(${item} ${ARGN})
  endif()
endfunction()

function(sdk_link_interface_ifdef feature_toggle interface)
  if(${${feature_toggle}})
    target_link_libraries(${interface} INTERFACE sdk_interface)
  endif()
endfunction()

function(sdk_library_link_libraries_ifdef feature_toggle item)
  if(${${feature_toggle}})
     sdk_library_link_libraries(${item})
  endif()
endfunction()

function(sdk_linker_sources_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_linker_sources(${ARGN})
  endif()
endfunction()

function(sdk_library_add_dependencies_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_library_add_dependencies(${ARGN})
  endif()
endfunction()
...
function(sdk_library_sources_ifndef feature_toggle source)
  if(NOT ${feature_toggle})
    sdk_library_sources(${source} ${ARGN})
  endif()
endfunction()

function(sdk_sources_ifndef feature_toggle)
   if(NOT ${feature_toggle})
    sdk_sources(${ARGN})
  endif()
endfunction()

function(sdk_cc_option_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_cc_option(${ARGN})
  endif()
endfunction()

function(sdk_ld_option_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_ld_options(${ARGN})
  endif()
endfunction()

function(sdk_link_libraries_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_link_libraries(${ARGN})
  endif()
endfunction()

function(sdk_compile_options_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_compile_options(${ARGN})
  endif()
endfunction()

function(sdk_compile_definitions_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_compile_definitions(${ARGN})
  endif()
endfunction()

function(sdk_include_directories_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_include_directories(${ARGN})
  endif()
endfunction()

function(sdk_library_compile_definitions_ifndef feature_toggle item)
  if(NOT ${feature_toggle})
    sdk_library_compile_definitions(${item} ${ARGN})
  endif()
endfunction()

function(sdk_library_include_directories_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_library_include_directories(${ARGN})
  endif()
endfunction()

function(sdk_library_compile_options_ifndef feature_toggle item)
  if(NOT ${feature_toggle})
    sdk_library_compile_options(${item} ${ARGN})
  endif()
endfunction()

function(sdk_link_interface_ifndef feature_toggle interface)
  if(NOT ${feature_toggle})
    target_link_libraries(${interface} INTERFACE sdk_interface)
  endif()
endfunction()

function(sdk_library_link_libraries_ifndef feature_toggle item)
  if(NOT ${feature_toggle})
     sdk_library_link_libraries(${item})
  endif()
endfunction()

function(sdk_linker_sources_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_linker_sources(${ARGN})
  endif()
endfunction()

function(sdk_library_add_dependencies_ifndef feature_toggle)
  if(NOT ${feature_toggle})
    sdk_library_add_dependencies(${ARGN})
  endif()
endfunction()
...
function(target_cc_option_fallback target scope option1 option2)
  if(CONFIG_CPP)
    foreach(lang C CXX)
      ...
      # apply to ASM.
      sdk_check_compiler_flag(${lang} ${option1} check)
      if(${check})
    ...
    sdk_check_compiler_flag(C ${option1} check)
    if(${check})
...
function(target_ld_options target scope)
  ...
  foreach(option ${ARGN})
    ...
    sdk_check_compiler_flag(C "" ${check})
...
function(check_set_compiler_property)
  ...
  foreach(option ${COMPILER_PROPERTY_PROPERTY})
    ...
    if(CONFIG_CPP)
      sdk_check_compiler_flag(CXX "${option}" check)
...
# Usage:
#   assert(SDK_TOOLCHAIN_VARIANT "SDK_TOOLCHAIN_VARIANT not set.")
#
...
function(sdk_file)
  ...
  if((ARGC EQUAL 0) OR (NOT (ARGV0 IN_LIST file_options)))
    message(FATAL_ERROR "No <mode> given to `sdk_file(<mode> <args>...)` function,\n \
Please provide one of following: APPLICATION_ROOT, CONF_FILES")
  ...
  if(FILE_UNPARSED_ARGUMENTS)
      message(FATAL_ERROR "sdk_file(${ARGV0} <val> ...) given unknown arguments: ${FILE_UNPARSED_ARGUMENTS}")
  endif()
...
Relative paths are only allowed with `-D${ARGV1}=<path>`")
  ...
  if(FILE_CONF_FILES)
    if(DEFINED FILE_BOARD_REVISION AND NOT FILE_BOARD)
        message(FATAL_ERROR
          "sdk_file(${ARGV0} <path> BOARD_REVISION ${FILE_BOARD_REVISION} ...)"
          " given without BOARD argument, please specify BOARD"
        )
    endif()
...
# Usage:
#   sdk_file_copy(<oldname> <newname> [ONLY_IF_DIFFERENT])
#
# Sdk file copy extension.
# This function is similar to CMake function
# 'file(COPY_FILE <oldname> <newname> [ONLY_IF_DIFFERENT])'
# introduced with CMake 3.21.
#
# Because the minimal required CMake version with Sdk is 3.20, this function
# is not guaranteed to be available.
#
# When using CMake version 3.21 or newer 'sdk_file_copy()' simply calls
...
function(sdk_file_copy oldname newname)
  set(options ONLY_IF_DIFFERENT)
  cmake_parse_arguments(SDK_FILE_COPY "${options}" "" "" ${ARGN})

  if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.21.0)
    if(SDK_FILE_COPY_ONLY_IF_DIFFERENT)
...
# Usage:
#   sdk_string(<mode> <out-var> <input> ...)
#
# Sdk string function extension.
...
function(sdk_string)
  ...
  cmake_parse_arguments(SDK_STRING "${options}" "" "" ${ARGN})

  if (NOT SDK_STRING_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "Function sdk_string() called without a return variable")
  endif()

  list(GET SDK_STRING_UNPARSED_ARGUMENTS 0 return_arg)
  list(REMOVE_AT SDK_STRING_UNPARSED_ARGUMENTS 0)

  list(JOIN SDK_STRING_UNPARSED_ARGUMENTS "" work_string)

  if(SDK_STRING_SANITIZE)
    string(REGEX REPLACE "[^a-zA-Z0-9_]" "_" work_string ${work_string})
  endif()

  if(SDK_STRING_TOUPPER)
...
# Usage:
#    sdk_list(TRANSFORM <list> <ACTION>
#                [OUTPUT_VARIABLE <output variable])
#
# Example:
#
#    sdk_list(TRANSFORM my_input_var NORMALIZE_PATHS
#                OUTPUT_VARIABLE my_input_as_list)
#
# Like CMake's list(TRANSFORM ...). This is intended as a placeholder
# for storing current and future Sdk-related extensions for list
# processing.
#
...
function(sdk_list transform list_var action)
  # Parse arguments.
  ...
  cmake_parse_arguments(SDK_LIST "" "${single_args}" "" ${ARGN})
  if(DEFINED SDK_LIST_OUTPUT_VARIABLE)
    set(out_var ${SDK_LIST_OUTPUT_VARIABLE})
  else()
...
# Usage:
#   sdk_var_name(<variable> <scope> <out>)
#
...
function(sdk_var_name variable scope out)
...
# Usage:
#   sdk_get(<variable> [MERGE [REVERSE]] [SYSBUILD [LOCAL|GLOBAL]] [VAR <var1> ...])
#
...
# sdk_get(FOO VAR FOO_A FOO_B)
# sdk_get(FOO MERGE VAR FOO_A FOO_B)
#
# sdk_get() is a common function to provide a uniform way of supporting
...
# - Environment
# - Locally in CMakeLists.txt before 'find_package(Sdk)'
#
# For example, if SDK_TOOLCHAIN_VARIANT is set in environment but locally
# overridden by setting SDK_TOOLCHAIN_VARIANT directly in the CMake cache
# using `-DSDK_TOOLCHAIN_VARIANT=<val>`, then the value from the cache is
# returned.
function(sdk_get variable)
  ...
  if(DEFINED GET_VAR_SYSBUILD)
    if(NOT ("${GET_VAR_SYSBUILD}" STREQUAL "GLOBAL" OR
      ...
      message(FATAL_ERROR "sdk_get(... SYSBUILD) requires GLOBAL or LOCAL.")
    endif()
  else()
  ...
  if(GET_VAR_REVERSE AND NOT GET_VAR_MERGE)
    message(FATAL_ERROR "sdk_get(... REVERSE) missing a required argument: MERGE")
  endif()
  ...
  foreach(scope IN LISTS scopes)
    foreach(var ${GET_VAR_VAR})
      sdk_var_name("${var}" "${scope}" expansion_var)
...
endfunction(sdk_get variable)

# Usage:
#   sdk_create_scope(<scope>)
#
# Create a new scope for creation of scoped variables.
#
# <scope>: Name of new scope.
#
function(sdk_create_scope scope)
  if(TARGET ${scope}_scope)
    message(FATAL_ERROR "sdk_create_scope(${scope}) already exists.")
  endif()
...
# Usage:
#   sdk_set(<variable> <value> SCOPE <scope> [APPEND])
#
# Sdk extension of CMake set which allows a variable to be set in a specific
# scope. The scope is used on later sdk_get() invocation for precedence
...
#
function(sdk_set variable)
  cmake_parse_arguments(SET_VAR "APPEND" "SCOPE" "" ${ARGN})

  sdk_check_arguments_required_all(sdk_set SET_VAR SCOPE)

  if(NOT TARGET ${SET_VAR_SCOPE}_scope)
    message(FATAL_ERROR "sdk_set(... SCOPE ${SET_VAR_SCOPE}) doesn't exists.")
  endif()
...
# Usage:
#   sdk_check_cache(<variable> [REQUIRED])
#
...
#   - In the project CMakeLists.txt before `find_package(Sdk)`.
#
...
#   cache as CACHED_<variable>. This allows the Sdk build system to detect
...
#
function(sdk_check_cache variable)
  ...
  if(CACHE_VAR_WATCH)
    ...
    sdk_boilerplate_watch(${variable})
  endif()
endfunction(sdk_check_cache variable)


# Usage:
#   sdk_boilerplate_watch(SOME_BOILERPLATE_VAR)
#
# Inform the build system that SOME_BOILERPLATE_VAR, a variable
# handled in the Sdk package's boilerplate code, is now fixed and
...
function(sdk_boilerplate_watch variable)
  variable_watch(${variable} sdk_variable_set_too_late)
endfunction()

function(sdk_variable_set_too_late variable access value current_list_file)
...
"
   ...
   *
   * Hint: ${variable} must be set before calling find_package(Sdk ...).
   *
   **********************************************************************
")
  endif()
...
# Usage:
#   sdk_get_targets(<directory> <types> <targets>)
#
...
function(sdk_get_targets directory types targets)
...
# Usage:
#   sdk_dt_preprocess(CPP <path> [<argument...>]
#                        SOURCE_FILES <file...>
#                        OUT_FILE <file>
#                        [DEPS_FILE <file>]
#                        [EXTRA_CPPFLAGS <flag...>]
...
#
# This allows Sdk developers to define linker sections and their content and
...
# Usage:
#   sdk_linker([FORMAT <format>]
#     [ENTRY <entry symbol>]
#   )
#
# Sdk linker general settings.
...
function(sdk_linker)
  ...
  if(LINKER_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "sdk_linker(${ARGV0} ...) given unknown "
  ...
  if(DEFINED LINKER_FORMAT)
    ...
    if(format_defined)
      message(FATAL_ERROR "sdk_linker(FORMAT ...) already configured.")
    else()
  ...
  if(DEFINED LINKER_ENTRY)
    ...
    if(entry_defined)
      message(FATAL_ERROR "sdk_linker(ENTRY ...) already configured.")
    else()
...
# Usage:
#   sdk_linker_memory(NAME <name> START <address> SIZE <size> FLAGS <flags>)
#
# Sdk linker memory.
...
function(sdk_linker_memory)
  ...
  foreach(arg ${single_args})
    if(NOT DEFINED MEMORY_${arg})
      message(FATAL_ERROR "sdk_linker_memory(${ARGV0} ...) missing required "
                          "argument: ${arg}"
      )
    endif()
  endforeach()
  ...
  set(MEMORY)
  sdk_linker_arg_val_list(MEMORY "${single_args}")
...
# Usage:
#   sdk_linker_memory_ifdef(<setting> NAME <name> START <address> SIZE <size> FLAGS <flags>)
#
...
#            sdk_linker_memory()
#
# See sdk_linker_memory() description for other supported arguments.
#
macro(sdk_linker_memory_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_linker_memory(${ARGN})
  endif()
endmacro()

# Usage:
#   sdk_linker_dts_section(PATH <path>)
#
# Sdk linker devicetree memory section from path.
#
...
function(sdk_linker_dts_section)
  ...
  dt_prop(name PATH ${DTS_SECTION_PATH} PROPERTY "sdk,memory-region")
  if(NOT DEFINED name)
    message(FATAL_ERROR "sdk_linker_dts_section(${ARGV0} ...) missing "
                        "\"sdk,memory-region\" property"
    )
  endif()
  sdk_string(SANITIZE name ${name})

  dt_reg_addr(addr PATH ${DTS_SECTION_PATH})

  sdk_linker_section(NAME ${name} ADDRESS ${addr} VMA ${name} TYPE NOLOAD)

endfunction()

# Usage:
#   sdk_linker_dts_memory(PATH <path> FLAGS <flags>)
#   sdk_linker_dts_memory(NODELABEL <nodelabel> FLAGS <flags>)
#   sdk_linker_dts_memory(CHOSEN <prop> FLAGS <flags>)
#
# Sdk linker devicetree memory.
...
#                  The flags r and x, or w and x may be combined like: rx, wx.
#
function(sdk_linker_dts_memory)
  ...
  if(DTS_MEMORY_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "sdk_linker_dts_memory(${ARGV0} ...) given unknown "
  ...
  if((DEFINED DTS_MEMORY_PATH AND (DEFINED DTS_MEMORY_NODELABEL OR DEFINED DTS_MEMORY_CHOSEN))
    ...
    message(FATAL_ERROR "sdk_linker_dts_memory(${ARGV0} ...), only one of "
  ...
  dt_prop(name PATH ${DTS_MEMORY_PATH} PROPERTY "sdk,memory-region")
  if(NOT DEFINED name)
    message(FATAL_ERROR "sdk_linker_dts_memory(${ARGV0} ...) missing "
                        "\"sdk,memory-region\" property"
    )
  endif()
  sdk_string(SANITIZE name ${name})
...
# Usage:
#   sdk_linker_group(NAME <name> [VMA <region|group>] [LMA <region|group>] [SYMBOL <SECTION>])
#   sdk_linker_group(NAME <name> GROUP <group> [SYMBOL <SECTION>])
#
# Sdk linker group.
...
#
# Example:
#   sdk_linker_memory(NAME memA START ... SIZE ... FLAGS ...)
#   sdk_linker_group(NAME groupA LMA memA)
#   sdk_linker_group(NAME groupB LMA groupA)
#
...
#   sdk_linker_memory(NAME memA START ... SIZE ... FLAGS ...)
#   sdk_linker_group(NAME groupA LMA memA)
#   sdk_linker_group(NAME groupB GROUP groupA)
#
...
function(sdk_linker_group)
  ...
  if(GROUP_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "sdk_linker_group(${ARGV0} ...) given unknown "
                        "arguments: ${GROUP_UNPARSED_ARGUMENTS}"
    )
  endif()

  if(DEFINED GROUP_GROUP AND (DEFINED GROUP_VMA OR DEFINED GROUP_LMA))
    message(FATAL_ERROR "sdk_linker_group(GROUP ...) cannot be used with "
                        "VMA or LMA"
    )
  endif()
  ...
  if(DEFINED GROUP_SYMBOL)
    if(NOT ${GROUP_SYMBOL} IN_LIST symbol_values)
      message(FATAL_ERROR "sdk_linker_group(SYMBOL ...) given unknown value")
    endif()
  endif()

  set(GROUP)
  sdk_linker_arg_val_list(GROUP "${single_args}")
...
# Usage:
#   sdk_linker_section(NAME <name> [GROUP <group>]
...
#   )
#
# Sdk linker output section.
# This function specifies an output section for the linker.
#
# When using sdk_linker_section(NAME <name>) an output section with <name>
...
#                       `sdk_linker_section_configure()` must be used.
...
function(sdk_linker_section)
  ...
  if(SECTION_UNPARSED_ARGUMENTS)
    message(WARNING "sdk_linker_section(${ARGV0} ...) given unknown "
                    "arguments: ${SECTION_UNPARSED_ARGUMENTS}"
    )
  endif()

  if(DEFINED SECTION_GROUP AND (DEFINED SECTION_VMA OR DEFINED SECTION_LMA))
    message(FATAL_ERROR "sdk_linker_section(GROUP ...) cannot be used with "
                        "VMA or LMA"
    )
  endif()
  ...
  if(DEFINED SECTION_PASS)
    ...
    if(${pass_length} GREATER 1)
      ...
      if((NOT (${pass_elem_0} STREQUAL "NOT")) OR (${pass_length} GREATER 2))
        message(FATAL_ERROR "sdk_linker_section(PASS takes maximum "
  ...
  set(SECTION)
  sdk_linker_arg_val_list(SECTION "${single_args}")
  sdk_linker_arg_val_list(SECTION "${options}")
  sdk_linker_arg_val_list(SECTION "${multi_args}")
...
# Usage:
#   sdk_linker_section_ifdef(<setting>
...
#            sdk_linker_section()
#
# See sdk_linker_section() description for other supported arguments.
#
macro(sdk_linker_section_ifdef feature_toggle)
  if(${${feature_toggle}})
    sdk_linker_section(${ARGN})
  endif()
endmacro()

# Usage:
#   sdk_iterable_section(NAME <name> [GROUP <group>]
#                           [VMA <region|group>] [LMA <region|group>]
...
function(sdk_iterable_section)
  ...
  if(NOT DEFINED SECTION_NAME)
    message(FATAL_ERROR "sdk_iterable_section(${ARGV0} ...) missing "
...
# Usage:
#   sdk_linker_section_obj_level(SECTION <section> LEVEL <level>)
#
...
#
# For example: sdk_linker_section_obj_level(SECTION init LEVEL PRE_KERNEL_1)
...
# Usage:
#   sdk_linker_section_configure(SECTION <section> [ALIGN <alignment>]
...
# attribute, call:
#   sdk_linker_section_configure(SECTION bar INPUT foo KEEP)
#
...
#                       using priority Sdk CMake linker will create sections
...
#
function(sdk_linker_section_configure)
  ...
  if(SECTION_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "sdk_linker_section_configure(${ARGV0} ...) given unknown arguments: ${SECTION_UNPARSED_ARGUMENTS}")
  endif()

  if(DEFINED SECTION_SYMBOLS)
    list(LENGTH SECTION_SYMBOLS symbols_count)
    if(${symbols_count} GREATER 2)
      message(FATAL_ERROR "sdk_linker_section_configure(SYMBOLS [start_sym [end_sym]]) takes maximum two symbol names (start and end).")
  ...
  if(DEFINED SECTION_PASS)
    ...
    if(${pass_length} GREATER 1)
      list(GET SECTION_PASS 0 pass_elem_0)
      if((NOT (${pass_elem_0} STREQUAL "NOT")) OR (${pass_length} GREATER 2))
        message(FATAL_ERROR "sdk_linker_section_configure(PASS takes maximum "
          "a single argument of the form: '<pass name>' or 'NOT <pass_name>'.")
      endif()
    endif()
  endif()
...
# Usage:
#   sdk_linker_symbol(SYMBOL <name> EXPR <expr>)
#
...
#     sdk_linker_symbol(SYMBOL bar EXPR "(@foo@ + 1024)")
#
function(sdk_linker_symbol)
  set(single_args "EXPR;SYMBOL")
  cmake_parse_arguments(SYMBOL "" "${single_args}" "" ${ARGN})

  if(SECTION_UNPARSED_ARGUMENTS)
    message(WARNING "sdk_linker_symbol(${ARGV0} ...) given unknown "
                    "arguments: ${SECTION_UNPARSED_ARGUMENTS}"
    )
  endif()

  set(SYMBOL)
  sdk_linker_arg_val_list(SYMBOL "${single_args}")
...
# Internal helper macro for sdk_linker*() functions.
...
# list later.
macro(sdk_linker_arg_val_list list arguments)
...
#
# Usage:
#   sdk_check_arguments_required(<function_name> <prefix> <arg1> [<arg2> ...])
#
macro(sdk_check_arguments_required function prefix)
  set(check_defined DEFINED)
  sdk_check_flags_required(${function} ${prefix} ${ARGN})
  set(check_defined)
endmacro()
...
#
# Usage:
#   sdk_check_flags_required(<function_name> <prefix> <flag1> [<flag2> ...])
#
macro(sdk_check_flags_required function prefix)
...
#
# Usage:
#   sdk_check_arguments_required_all(<function_name> <prefix> <arg1> [<arg2> ...])
#
macro(sdk_check_arguments_required_all function prefix)
  foreach(required ${ARGN})
...
#
# Usage:
#   sdk_check_arguments_exclusive(<function_name> <prefix> <arg1> <arg2> [<arg3> ...])
#
macro(sdk_check_arguments_exclusive function prefix)
  set(check_defined DEFINED)
  sdk_check_flags_exclusive(${function} ${prefix} ${ARGN})
  set(check_defined)
endmacro()
...
#
# Usage:
#   sdk_check_flags_exclusive(<function_name> <prefix> <flag1> <flag2> [<flag3> ...])
#
macro(sdk_check_flags_exclusive function prefix)
  set(args_defined)
  foreach(arg ${ARGN})
...
function(application_name_set name)
  set(APPLICATION_NAME ${name} CACHE STRING "Full application name." FORCE)
  set(APPLICATION_NAME_ELF ${name}.elf CACHE STRING "Full application exe name." FORCE)
endfunction()
